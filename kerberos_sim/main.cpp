...
int main() {
    // Tạo khóa ngẫu nhiên
    vector<unsigned char> key = Encryption::GenerateRandomKey();

    // Hiển thị khóa ở dạng hex
    cout << "Generated Key (Hex): ";
    ...
    // Mã hóa dữ liệu
    string ciphertext = Encryption::Encrypt(plaintext, key);
    ...
    // Giải mã dữ liệu
    string decryptedText = Encryption::Decrypt(ciphertext, key);
    ...
    if (decryptedText == plaintext) {
        ...
        cerr << "[ERROR] Decryption does not match original plaintext!" << endl;
    }
